...
class ChannelController extends AdminController {
    ...
    public function add(){
        if(IS_POST){
			...
			$menus = M('Category')->field('title,name,id,pid')->order('sort asc')->select();
	...
	//获取分类的信息
	public function venuenum()
	{
		$id = i('post.id', 0);
		$result=M('Category')->field('title,name,id,pid')->where("id=".$id)->order('sort asc')->find();
