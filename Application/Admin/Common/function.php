...
function get_attribute_type($type=''){
    ...
    static $_type = array(
		...
		'multiple'    	=>  array('多图上传','varchar(255) NOT NULL'),
...
function get_sub_category($pid=0){
	$catlist=M('Category')->field('id,title')->where("pid=".$pid)->order('sort asc,id desc')->select();
	$catstr=array();
	foreach($catlist as $row)
	{
		$catstr[$row['id']]=$row['title'];
	}
    return  $catstr;
}
...
function parse_field_attr($string) {
	...
	{
		$catstr=get_sub_category(substr($string,1,-1));
		return $catstr;
	}
    $array = preg_split('/[,;\r\n]+/', trim($string, ",;\r\n"));
