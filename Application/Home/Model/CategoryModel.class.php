...
class CategoryModel extends Model{
	...
	public function getpidlist($id, $field = true){

		$info = $this->info($id, 'id');
		$catres=$this->where("pid=".$info['id'])->find();
		$cattid=$this->where("id=".$info['id'])->find();
		if($catres)
		{
			$map = array('pid' => $info['id'], 'status' => 1);
		}else
		{
			$map = array('pid' => $cattid['pid'], 'status' => 1);
			if($cattid['pid']==0)
			{
			$map['template_lists']=array('not in',array('about','distribution'));
			}
		}

		 return $this->field($field)->where($map)->order('sort')->select();
		//echo $this->field($field)->getLastSql();
	}
