...
class Article extends TagLib{
	...
	protected $tags   =  array(
		...
		'wappage'  => array('attr' => 'cate,listrow', 'close' => 0), //手机列表分页
		'page'     => array('attr' => 'cate,listrow', 'close' => 0), //列表分页
		'position' => array('attr' => 'pos,cate,limit,filed,name', 'close' => 1), //获取推荐位列表
		'list'     => array('attr' => 'name,category,child,page,row,field,orderby', 'close' => 1), //获取指定分类列表
	);
	...
	public function _list($tag, $content){
		...
		$parse .= '$category, \''.$orderby.'`level` DESC,`create_time` DESC\', 1,';
		$parse .= $field . ');';
		$parse .= ' ?>';
	...
	/* 列表数据分页 */
	public function _wappage($tag){
		$cate    = $tag['cate'];
		$listrow = $tag['listrow'];
		$parse   = '<?php ';
		$parse  .= '$__PAGE__ = new \Think\Wappage(get_list_count(' . $cate . '), ' . $listrow . ');';
		$parse  .= 'echo $__PAGE__->show();';
		$parse  .= ' ?>';
		return $parse;
	}
